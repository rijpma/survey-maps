...
This repository contains a suite of tools for downloading, reconstructing, and creating machine learning datasets from historical map tiles (specifically from the National Library of Scotland - NLS). 

The pipeline consists of three main scripts that handle data acquisition, visual reconstruction, and manual segmentation (labeling).

## Pipeline Overview and Data Flow

Data flows linearly through the project, starting from remote web servers down to local ML-ready datasets:

1. **`nls.py` (Acquisition)**: Pulls individual 256x256 tiles from the web into a structured grid format.
2. **`stitch.py` (Reconstruction)**: Combines those individual tiles into a single, cohesive, high-resolution map for viewing.
3. **`label.py` (Dataset Generation)**: Samples a subset of the downloaded tiles to create pairs of images and binary masks for training computer vision models.
